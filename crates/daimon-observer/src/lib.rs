...
//!    durations, etc.).
//!
//! Both paths write to `observer.metrics`. Threshold-bound named queries
//! emit `AnomalyDetected` rows into `observer.anomalies` + (Phase 8) emit
//! a bus envelope subscribed by Guard + Orchestrator.
