...
pub fn Vms() -> impl IntoView {
    ...
    let vms = Resource::new(
        move || cluster_id(),
        |cid| get_cluster_vms(cid),
    );

    view! {
        <Suspense fallback=|| view! { <div class="text-text-muted text-sm">"Loading VMs..."</div> }>
            {move || vms.get().map(|result| match result {
